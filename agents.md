...
to Vercel and to future loop ticks. The autonomous
...
`pnpm deploy:check` polls Vercel for the deploy
...
### 6. Hardline UX: the user is never quizzed.

Every question to the user accepts a 1-word or 1-sentence
answer. No checkpoint surfaces more than 5 questions. The
agents do the thinking; the user supplies the pitch and the
occasional accept/redirect. This is product policy, baked into
every phase that touches the in-session UI.

### 7. Content stays in `personas/` and `templates/`. Data stays in Postgres.

`personas/*.md` and `templates/*.json` are file-as-source and
ship via PR. Session state, transcripts, artifacts, and audit
rows live in Supabase Postgres. No hardcoded copy in components.
No hardcoded persona/template data in code. Every persona /
template change is a PR.

---

## Project

**boardroom** — AI-orchestrated board-room simulator: pitch in, spec out. Lives at https://boardroom-breakdown.vercel.app.
...
components/          UI components grouped by family (site/, boardroom/, etc.).
lib/                 Pure helpers; Supabase clients; persona/template loaders.
personas/            Persona definitions (markdown + frontmatter). File-as-source.
templates/           Discussion templates (JSON). File-as-source.
config/              Runtime limits (token caps, rate-limits, quotas).
...
design/              Design exports (commissioned per claude-design.prompt.md).
setup/               Per-external-service runbooks (Vercel, Supabase, etc.).
scripts/             Verify + deploy gates; data validation.
e2e/                 Playwright specs (hermetic against alt port 4011).
__tests__/           Vitest unit specs.
```
...
| Agent | Use for |
|---|---|
| `scout` | Open-web research with citations (Next.js / Supabase / Anthropic docs, model IDs, version pins). |
| `reader` | Fresh-eyes site observer; walks the authed app with the session cookie. |
| `persona-steward` | Validate or refine a persona definition under `personas/`. Returns a diff + rationale. |
...
### `VERCEL_TOKEN` (+ `VERCEL_PROJECT_ID`) — deploy gate

Used by `pnpm deploy:check` to read deploy state for the
project's Vercel deployments.
...
Find the project id: Vercel dashboard → project settings → General.

If missing, `pnpm deploy:check` exits 3 with a clear error.

### `SUPABASE_*` — Postgres + Auth (phases 2–3+)

```
SUPABASE_URL=https://<project-ref>.supabase.co
SUPABASE_ANON_KEY=...
SUPABASE_SERVICE_ROLE_KEY=...                 # server-only; bypasses RLS
NEXT_PUBLIC_SUPABASE_URL=...                  # mirrors of the above for the
NEXT_PUBLIC_SUPABASE_ANON_KEY=...             # client bundle
```

Provisioning runbook: `setup/03_supabase.md`.

### `ANTHROPIC_API_KEY` — persona reasoning (phase 7)

```
ANTHROPIC_API_KEY=...
ANTHROPIC_MODEL=claude-opus-4-7               # default; per-persona override allowed
```

Provisioning runbook: `setup/04_anthropic.md`.

### `OPENAI_API_KEY` — moderation only (phase 8)
...
Used exclusively for input + output moderation pre-filter; no
generation calls go to OpenAI. Provisioning runbook:
`setup/05_openai.md`.
