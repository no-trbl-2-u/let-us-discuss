---
name: jot
description: Quickfire user observation → one row appended to `plan/CRITIQUE.md`. Decide-and-ship in seconds; no questions back; no verify gate; no deploy gate. The next `/iterate` tick scores the jot against pending work (user-source findings get a +0.5 score bump). Use when the user spots something on the live site and wants to drop a note — `/jot <text>`, `/jot --url <path> <text>`, `/jot --severity high <text>`.
---
