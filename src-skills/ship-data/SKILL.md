---
name: ship-data
description: Add, repair, or normalize one entity in the project's structured data layer. Pattern-agnostic (GitHub-as-DB, external DB, SaaS data store, hybrid). Enforces a provenance marker on every record. Use when `data/BACKLOG.md` has a pending row, or when the user wants to add/update one data record — `/ship-data`, `/ship-data add <entity> <slug>`, "ship the next data record".
---
