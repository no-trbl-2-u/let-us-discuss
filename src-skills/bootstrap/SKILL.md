---
name: bootstrap
description: Provision external resources (GitHub repo, hosting project, database project) from tokens-in to green-deploy + ticking cloud loop. Authorized executor; reads `setup/00_files.md` + `setup/NN_<service>.md` runbooks; writes secrets to `.env` / GitHub Actions / hosting env vars; may pause for AskUserQuestion on inherently-human handoffs. NOT invoked under `/loop` or `/march`. Use when setting up a fresh project's external services — `/bootstrap`, `/bootstrap with manifest`, `/bootstrap <service>`, `/bootstrap rotate <service>`.
---
...
Provisioned:
  ...
  - Vercel project <PROJECT> (region)
  - Supabase project <PROJECT> (region)
