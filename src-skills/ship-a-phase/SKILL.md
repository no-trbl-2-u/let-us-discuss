---
name: ship-a-phase
description: Ship one phase from `plan/steps/01_build_plan.md` end-to-end — code + tests + verify gate + commit + push + deploy. Reads `plan/phases/phase_<N>_<topic>.md` for the brief; falls through to `/plan-a-phase` if the brief is missing. Use when there's an unchecked `[ ]` phase row and the user wants it built — `/ship-a-phase`, "ship the next phase", "build phase N".
---
...
`plan/steps/01_build_plan.md` carves the project into ~10–20 phases.
...
**Brief:** [\`plan/phases/phase_<N>_<topic>.md\`](https://github.com/<YOUR_ORG>/<YOUR_REPO>/blob/main/plan/phases/phase_<N>_<topic>.md)
...
8. **Content stays in `personas/` and `templates/`.** Dynamic
   data stays in Supabase Postgres. No hardcoded copy/records
   in components.
9. **Hardline UX:** every user-facing question accepts a
   1-word or 1-sentence answer; no checkpoint exceeds 5
   questions. Persona / template changes ship via PR, not
   in-product authoring.
