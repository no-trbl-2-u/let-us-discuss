---
name: expand
description: Read accumulated signals (audit findings clustered, critique findings clustered, spec drift, design landings, triage backlog patterns, data growth, commit-pattern signals) and propose new phase candidates to `plan/PHASE_CANDIDATES.md`. Posture-controlled in `plan/bearings.md` — `bold` files candidates, `strict` is a no-op, `autonomous` writes directly to the build plan. Use when reality has outpaced the original plan and the user wants new phase ideas — `/expand`, or invoked by `/march` when its rate-limit window opens.
---
