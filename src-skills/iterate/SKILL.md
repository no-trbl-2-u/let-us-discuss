---
name: iterate
description: Audit the project, pick the highest-impact weakness, ship one improvement end-to-end. Drains `plan/CRITIQUE.md` + `plan/AUDIT.md` + triaged GitHub issues. Use when phase work is done but the project needs ongoing polish, when the user says "ship the next fix", "iterate", or invokes `/iterate`. Falls through to `/expand` when no actionable findings remain and posture is bold.
---
