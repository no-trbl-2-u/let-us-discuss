---
name: critique
description: External-observer pass. Visit the live site as a first-time reader (delegated to a `reader` sub-agent with browser tools), self-assess findings, file durable rows to `plan/CRITIQUE.md` for `/iterate` to drain. Rate-limited by `/march` (≥12 commits since last pass + green deploy + no pending HIGH finding). Use when the user wants a fresh-eyes audit of the deployed product — `/critique`, "run a critique", "what does a stranger see".
---
...
Agent({
  ...
  prompt: "Visit these URLs of https://<YOUR_PROD_URL>: [list].
