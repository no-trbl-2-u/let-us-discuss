---
name: triage
description: Process unlabeled GitHub issues — classify (bug/enhancement/content/data/docs/seo/a11y/perf), label (`triage:loop-queued` / `triage:needs-user` / `triage:closed` / `triage:reviewed`), comment, and route to the right backlog (`plan/AUDIT.md` / `data/BACKLOG.md` / `plan/steps/01_build_plan.md`). Cheapest gate in `/march`; exits in <1s when zero unlabeled issues. Use when there are open unlabeled issues or the user invokes `/triage`.
---
...
GH_REPO=<YOUR_ORG>/<YOUR_REPO>           # optional; defaults to <YOUR_ORG>/<YOUR_REPO>
```
...
GH_REPO=${GH_REPO:-<YOUR_ORG>/<YOUR_REPO>}
...
[plan/AUDIT.md](https://github.com/<YOUR_ORG>/<YOUR_REPO>/blob/main/plan/AUDIT.md)
