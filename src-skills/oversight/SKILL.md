---
name: oversight
description: User-in-the-loop course-correction. Pause autonomy, audit state, brief the user, ask 1-4 targeted questions via AskUserQuestion (the only skill outside `bootstrap` that asks anything), adjust the plan files in a single commit. Use when the user wants to redirect the loop, promote/reject phase candidates, sweep stale findings, or set a `Bias:` direction — `/oversight`, `/oversight phase`, `/oversight content`, `/oversight deploy`, `/oversight reset`.
---
