---
name: march
description: Outer dispatcher for the autonomous loop. Reads project state (unlabeled GitHub issues, critique queue, pending phases, data backlog, expand window) and delegates to the right shipping skill in priority order. Use when the user wants the loop to pick the right next action automatically — `/march`, `/loop /march`, "run the loop", "what's next".
---
