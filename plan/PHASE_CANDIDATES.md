...
> Last pass: 2026-05-21 at commit 7d8bb8e
> Pass count: 7
> Posture: bold
...
> **Pass-6 batch (expand 2026-05-21):** Three candidates from
> the post-pass-14 signal pile — 80+ commits since the last
> expand pass dominated by voice/copy fixes across parallel
> surfaces; recurring "regression" pattern where one fix
> missed a sibling surface in the same component file; five
> operator `[needs-user-call]` rows accumulated; 14 consecutive
> /critique passes with no mobile coverage because Chrome MCP
...
- proposed: 2026-05-21, expand pass 6
- source signals:
  - **14 consecutive /critique passes confirmed
    `mcp__claude-in-chrome__*` unavailable** to the reader
    sub-agent (WebFetch-only). Mobile reflow + interactive
    state + console + network timing all unreachable via
    the existing critique path.
  - **`[needs-user-call]` Chrome MCP row** sits permanently
    pending in `plan/CRITIQUE.md`; the operator gate hasn't
    moved across 14 passes.
  - **The page set /critique walks is small**: /, /try,
    /signin, /about, /about/personas, /legal/privacy +
    /legal/terms. Adding mobile e2e specs to each closes
    the gap with static guards.
- rationale: Rather than wait for the operator to wire
  Chrome MCP, the loop can close the same observation gap
  with deterministic mobile e2e tests. The mobile-reflow
  spec on /try (`e2e/try.spec.ts:41`) already proves the
  pattern works at 375px. Extending it across the public
  surfaces produces guards that fire in the verify gate
  rather than waiting for a critique pass to read mobile
  state.
- proposed scope: 1 phase.
  - `e2e/mobile/landing.mobile.spec.ts` + `about.mobile.spec.ts`
    + `about-personas.mobile.spec.ts` + `signin.mobile.spec.ts`
    + `legal-privacy.mobile.spec.ts` + `legal-terms.mobile.spec.ts`
    — each asserts (a) no horizontal scroll at 375×800,
    (b) the H1 is in-viewport at top of fold,
    (c) at least one key CTA is visible without scrolling.
  - Closes the [needs-user-call] Chrome MCP row's product
    impact (the gate stays operator-blocked for interactive
    states, but the static "is the page mobile-coherent"
    check is now verified on every commit).
- estimated phases: 1.
- conflicts: none. Composes with the existing
  `e2e/try.spec.ts` mobile pattern + the URL-contract
  smoke walker.
- next step: when promoted, brief from this row +
  `e2e/try.spec.ts:41` as the canonical mobile-reflow
  shape to copy.
